...
(define-language MyLLVM
  ...
  [lbl-i ::= variable-not-otherwise-mentioned integer UNDEFINED]
  ...
  [l-notphi ::=
     ...
     ; If the last label was lbl-i0, set reg-i to a-or-r0, else a-or-r1                                 ; l is a sequence of instructions
     UNDEFINED
     ]  
  [l ::=
     l-br                                  ; all branching instructions
...
(define-judgment-form  MyLLVM
 ...
 ; Matching key is not in empty dict
  [
   ----- "Lookup-Label-Empty"
   (label-list-lookup empty lbl-i UNDEFINED)]
  [(label-list-lookup lbl-list lbl-i UNDEFINED)
   (where #t (different lbl-i lbl-i_2))
   ----- "Lookup-Label-Not-Found"
   (label-list-lookup (lbl-list lbl-i_2 lbl-i_5) lbl-i UNDEFINED)]
  )
...
(define-judgment-form  MyLLVM
  ...
  [(label-lookup p main l)
   (-->l+ p R l main main R_2 (ret i32 reg-i) c_0 c_1)
